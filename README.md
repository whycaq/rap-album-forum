...
- 📋 **专辑列表页面** - 完整的专辑浏览功能，支持搜索、筛选、排序和分页
...
2. **专辑列表浏览**（⭐ 新功能）
   - 访问 `/albums` 查看所有专辑
   - **搜索功能**：在搜索框输入关键词搜索专辑名称或艺人
   - **筛选功能**：按流派（Hip-Hop、Rap、Trap等）和年份筛选
   - **排序功能**：支持按最新发布、评分最高、最热门排序
   - **分页功能**：支持每页12/24/48/96张专辑，可跳转到指定页
   - **响应式设计**：完美适配桌面端和移动端
   - 点击任意专辑卡片跳转到专辑详情页

3. **专辑详情页面**
   - 专辑信息展示（封面、标题、艺人、流派、年份）
   - 专辑曲目列表（支持播放单曲）
   - 专辑描述和发行信息
   - "评分与评论"入口，跳转到独立评论页面

4. **专辑评分评论**（⭐ 新功能）
   - 访问 `/album/:id/rating` 进入专辑评分评论页面
   - **评分功能**：
     - 查看专辑平均评分和评分人数
     - 登录用户可为专辑打5星评分
     - 实时显示评分文案提示
...
VITE_SUPABASE_URL=你的Supabase项目URL
VITE_SUPABASE_ANON_KEY=你的Supabase匿名密钥
VITE_SPOTIFY_CLIENT_ID=你的Spotify_Client_ID
VITE_SPOTIFY_CLIENT_SECRET=你的Spotify_Client_Secret
```

获取 Spotify 凭证：访问 [Spotify开发者控制台](https://developer.spotify.com/dashboard)

### 使用管理后台导入

1. 启动项目：`npm run dev`
2. 访问：`http://localhost:3000/admin/spotify-import`
3. 搜索并导入你喜欢的专辑
